...
export function subdigestLeaves(topology: ConfigTopology): string[] {
  return leavesOf(topology).filter((l) => isSubdigestLeaf(l)).map((l) => (l as SubdigestLeaf).subdigest)
}

export function toSimplifiedConfig(config: WalletConfig): SimplifiedWalletConfig {
  const leaves = leavesOf(config.topology).filter(isSignerLeaf)
...
export function subdigestOf(wallet: string, digest: ethers.BytesLike, chainId: ethers.BigNumberish) {
  return ethers.utils.keccak256(
    ethers.utils.solidityPack(
